...
// need refactor: the application wont any external services like firebase or supabase for authentication.
// remove the firebase logic here later
      ...
      async (error) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          ...
          if (currentUser) {
            ...
            console.error("Unauthorized access - no user logged in.");
            Cookies.remove("auth-token");
            if (typeof window !== "undefined") {
              window.location.href =
                "/signin?redirect=" + window.location.pathname;
